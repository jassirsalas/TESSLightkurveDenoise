...
def contaminants(tpf, mask='pipeline', dmag=5, dist_cont=100):
    ...
    query = query[query['dist'] < dist_cont]['source_id','ra','dec','parallax','pmra','pmdec','phot_g_mean_mag','dist']
...
def lc_to_perid(lc, oversample_factor=1, output_path='default',xmax=20):
...
def query_for_spectral_type(identifier, only_first_two_characters=True,
    ...
    if len(result['SP_TYPE']) > 0:
        ...
        if only_first_two_characters:
            return result['SP_TYPE'][0][:15].strip()
        else:
